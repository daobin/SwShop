...
class PaypalHelper
{
    ...
    public function orderProcess(array $customerInfo, array $addressInfo, array $paymentInfo): array
    {
        ...
        if ($add) {
            return ['url' => $this->paymentInfo['cfg_list']['PAYPAL_CHECKOUT_URL'] . '?token=' . $token];
        }
    ...
    {
        $basicAuth = $this->paymentInfo['cfg_list']['PAYPAL_API_CLIENT_ID'] . ':' . $this->paymentInfo['cfg_list']['PAYPAL_API_SECRET'];

        $chOptions = [
            CURLOPT_URL => $this->paymentInfo['cfg_list']['PAYPAL_API_URL'],
            CURLOPT_HTTPHEADER => [
        ...
        switch ($service) {
            ...
            // 支付订单
            case 'capture_order':
                $chOptions[CURLOPT_URL] .= '/v2/checkout/orders/%s/capture';
                $chOptions[CURLOPT_POST] = true;
                break;
                break;
            default:
