<?php
\App\Helper\TemplateHelper::widget('sp_admin', 'header', ['add_url' => '/spadmin/shipping/0', 'timestamp' => $timestamp ?? '']);
?>
    <div class="layui-fluid hd-padding-top60">
        <table id="shipping_list" lay-filter="opt"></table>
    </div>
    ...
    <script>
        layui.use(['table'], function () {
            layui.table.render({
                ...
                cols: [
                    [
                        {field: 'method_name', title: '货运方式', align: 'center'},
                        {field: 'method_code', title: '货运编码', align: 'center'},
                        {field: 'note', title: '货运备注', align: 'center'},
                ...
                switch (obj.event) {
                    case 'edit':
                        let layer_idx = layer.open({
                            ...
                            title: '编辑',
                            skin: 'hd-open-edit',
                            content: '/spadmin/shipping/' + obj.data.method_code
                        });
